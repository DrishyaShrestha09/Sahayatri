...
const postACampaign = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ message: "Campaign posted successfully", campaign: newCampaign });
  } catch (error) {
...
const getAllCampaigns = async (req, res) => {
  try {
    ...
    // Calculates total fund raised
    const totalFundRaised = campaigns.reduce(
      (sum, campaign) => sum + campaign.fundRaised,
      0
    );

    res.status(200).send({ campaigns, totalFundRaised });
  } catch (error) {
